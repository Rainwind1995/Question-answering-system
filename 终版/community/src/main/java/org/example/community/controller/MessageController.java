...
public class MessageController implements CommunityConstant {
    ...
    public String sendLetter(String toName, String content) {
        ...
        // 插入会话id, 我们定义的规则是哪个id小就在前面: fromId_toId
    ...
    public String getChatPage(Model model) {
        ...
        User user = hostHolder.getUser();
        // 会话列表
        List<Message> conversationList = messageService.findChatConversations(user.getId());
        List<Map<String, Object>> conversations = new ArrayList<>();
        if (conversationList != null) {
            for (Message message : conversationList) {
                Map<String, Object> map = new HashMap<>();
    ...
    // 显示单个会话
    @RequestMapping(path = "/chat/detail/{conversationId}", method = RequestMethod.GET)
    @ResponseBody
    public String getChatDetail(@PathVariable("conversationId") String conversationId) {
        // 获取私信列表
        List<Message> chatList = messageService.findChats(conversationId);
        ...
        // 设置已读
        List<Integer> ids = getLetterIds(chatList);
        if (!ids.isEmpty()) {
            messageService.readMessage(ids);
        }
    ...
    public String getNoticeList(Model model){
        ...
        if(message != null){
            ...
            // 将 message 中的 content 里面的转义字符去掉
